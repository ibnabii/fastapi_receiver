...
from src.workers.schemas import WorkerCreate, WorkerRead

router = APIRouter()


@router.get("/", description="All workers, for admin only", response_model=list[WorkerRead])
...
@router.post("/", description="Create a new Worker", status_code=status.HTTP_201_CREATED)
